...
SELECT
    CASE
        WHEN EXTRACT(YEAR FROM AGE(outcomes.dob, outcomes.ts)) < 1 THEN 'Kitten'
        WHEN EXTRACT(YEAR FROM AGE(outcomes.dob, outcomes.ts)) >= 1 AND EXTRACT(YEAR FROM AGE(outcomes.dob, outcomes.ts)) <= 10 THEN 'Adult'
        WHEN EXTRACT(YEAR FROM AGE(outcomes.dob, outcomes.ts)) > 10 THEN 'Senior'
    END AS age_category,
    COUNT(*) AS count
FROM outcomes
WHERE outcomes.outcome_type = 'Adopted'
